...
function _func_rosserver() {
    if [ -z "$enp3s0_addr" ]; then
        echo "No enp3s0 connection..."
        ...
        echo "There are enp3s0 connection"
        export ROS_MASTER_URI=http://${enp3s0_addr}:11311
        export ROS_HOST_NAME=${enp3s0_addr}
        export ROS_IP=${enp3s0_addr}
        export PS1="\[\033[41;1;33m\]<ROS_server>\[\033[0m\]\w$ "
    fi
...
function _func_rosaddress() {
    # Get now enp3s0 or wlan0 IP address
    enp3s0_addr=$(ip -f inet -o addr show enp3s0|cut -d\  -f 7 | cut -d/ -f 1)
